...
void printLatLongToUTMCPP(const std::string& city, double lat, double lon) {
    ...
    std::vector<double> utmResult = convertLatLongToUTM(lat, lon);
    ...
    start = std::chrono::high_resolution_clock::now();
    std::vector<double> latLongResult = convertUTMToLatLong(utmResult[0], utmResult[1], static_cast<int>(utmResult[2]), std::string(1, static_cast<char>(utmResult[3])));
    end = std::chrono::high_resolution_clock::now();
    duration_ms = end - start;
...
void printLatLongToUTMPython(const std::string& city, double lat, double lon) {
    ...
    start = std::chrono::high_resolution_clock::now();
    std::vector<double> utmResult = convertLatLongToUTMPython(lat, lon);
    end = std::chrono::high_resolution_clock::now();
    duration_ms = end - start;
    ...
    start = std::chrono::high_resolution_clock::now();
    std::vector<double> latLongResult = convertUTMToLatLongPython(utmResult[0], utmResult[1], static_cast<int>(utmResult[2]), std::string(1, static_cast<char>(utmResult[3])));
    end = std::chrono::high_resolution_clock::now();
    duration_ms = end - start;
